...
- 修复敏感近期来源可被非敏感 REM 摘要绕过审核并自动晋升的问题；Deep 事务现在会重新计算 Signal、Daily Memory 及未知来源的敏感性。
- 修复 Deep 在 UTC 边界误把同一本地日期当成两日证据的问题；证据日期遵守配置时区，只接受仍 active 且未过期的近期来源。
- 修复近期层永久删除未完整清理关联视图和隐私派生数据的问题；级联清除近期证据、raw turn、work item、准入决定、摘要来源、投影任务、REM、模型调用、召回与索引，并保留长期记忆生成 evidence-impact 审核。
- 长期记忆移入回收站时会在同一事务中使其关联的开放审核失效，回收站与待审核队列不再同时显示同一条记忆。
- 永久删除改为一次点击确认，不再要求手输确认文本；仍保留不可撤销提示、删除前备份与隐私清理。
- 新建项目等必填表单的“取消”与关闭按钮使用无验证提交，空白名称时可直接关闭；仅“保存”会校验必填项。
- 回收和永久删除成功后自动关闭旧记忆的详情抽屉，避免界面在操作已生效后仍显示过期状态。
- “历史版本”仅显示“历史与回收站”状态提示，不再误显示项目状态筛选项。
