...
const workspaceMeta = {
  ...
  library: ["记忆库", "长期记忆、近期信号、Daily Memory 与冻结历史"],
  projects: ["项目", "围绕项目摘要、当前状态、决定与开放问题工作"],
  reviews: ["审核", "高风险决定与日常整理建议分流处理"],
  system: ["系统", "Dream、召回、模型、存储、Hermes 原生文件和高级设置"],
};
...
function showSystem(tab) { systemTab = tab; $$('[data-system-tab]').forEach(b=>b.classList.toggle('active',b.dataset.systemTab===tab)); $$('.system-pane').forEach(p=>p.classList.toggle('active',p.id===`system-${tab}`)); if(tab==='hermes') loadHermesNativeMemory().catch(fail); }
function showLibrary(tab) { libraryTab = tab; $$('[data-library-tab]').forEach(b=>b.classList.toggle('active',b.dataset.libraryTab===tab)); $("#library-filter").innerHTML = tab === "memories" ? '<option value="active">当前</option><option value="trashed">回收站</option><option value="superseded">历史</option><option value="all">全部</option>' : tab === "recent" ? '<option value="active">活跃</option><option value="expired">已过期</option><option value="trashed">回收站</option><option value="all">全部</option>' : tab === "daily" ? '<option value="active">活跃</option><option value="expired">已过期</option><option value="trashed">回收站</option><option value="all">全部</option>' : '<option value="all">冻结的 v0.5 候选</option>'; loadLibrary().catch(fail); }
...
async function loadAll() {
  try {
    ...
    const loaders = [
      ["概览", loadOverview, ["#metrics", "#health", "#recent-activity"]],
      ["设置", loadSettings],
      ["记忆库", loadLibrary, ["#library-list"]],
      ["项目", loadProjects, ["#project-list"]],
      ["审核", loadReviews, ["#review-list"]],
      ["Dream", loadDreams, ["#dream-list"]],
      ["召回轨迹", loadTraces, ["#trace-list"]],
      ["模型调用", loadCalls, ["#call-list"]],
      ["存储", loadStorage, ["#storage-health"]],
      ["隔离会话", loadIngestionIssues, ["#ingestion-issues"]],
      ["备份", loadBackups, ["#backup-list"]],
    ];
    const results = await Promise.allSettled(loaders.map(([, loader]) => loader()));
    results.forEach((result, index) => {
      if (result.status === "fulfilled") {
        (loaders[index][2] || []).forEach((selector) => $(selector)?.classList.remove("skeleton"));
      }
    });
...
async function loadOverview() {
  ...
  const metrics = [["长期记忆",c.active_memories,"durable"],["近期信号",c.active_recent_signals,"14d pool"],["Daily Memory",c.active_daily_memories,"30d pool"],["REM 反思",c.active_rem_reflections,"pending deep"],["开放审核",c.open_reviews,"reviews"],["最近备份",(await api('/backups')).length,"archives"]];
  $("#metrics").innerHTML = metrics.map(([label,value,note])=>`<div class="metric"><span>${label}</span><b>${value ?? 0}</b><small>${note}</small></div>`).join("");
  const attention=[]; if(c.open_reviews) attention.push(`${c.open_reviews} 项敏感、冲突或删除影响需要处理`); if(c.active_rem_reflections) attention.push(`${c.active_rem_reflections} 条 REM 反思等待 Deep`); if(!s.storage?.ok) attention.push("存储投影需要维护");
  ...
  const runs=await api('/dream-runs?limit=5'); $("#recent-activity").innerHTML=runs.map(r=>`<div class="list-row"><div class="list-main"><div class="list-title">Dream · ${esc(r.status)}</div><div class="list-meta">${when(r.started_at)} · 信号 ${r.recent_count||0} · Daily ${r.daily_count||0} · REM ${r.reflection_count||0} · Deep ${r.promoted_count||0}</div></div>${badge(r.error?'blocked':'complete',r.error?'danger':'good')}</div>`).join('')||empty('还没有 Dream 记录');
}

async function loadLibrary() {
  const query=($("#library-search")?.value||"").toLowerCase(); const filter=$("#library-filter")?.value||"active"; let rows=[];
  if(libraryTab==='memories') {
    const statuses=filter==='all'?['active','superseded','trashed']:[filter];
    rows=(await Promise.all(statuses.map(s=>api(`/memories?status=${s}`)))).flat();
  } else if(libraryTab==='recent') {
    rows=await api(`/recent-signals?status=${filter==='all'?'':filter}`);
  } else if(libraryTab==='daily') {
    rows=await api(`/daily-memories?status=${filter==='all'?'':filter}`);
  } else {
    rows=(await Promise.all(['pending','expired','rejected','promoted'].map(s=>api(`/candidates?status=${s}&admission_state=legacy_history`)))).flat();
  }
  rows=rows.filter(r=>JSON.stringify(r).toLowerCase().includes(query));
  $("#library-list").innerHTML=rows.map(r=>{
    if(libraryTab==='memories') return `<div class="list-row" data-open="memory" data-id="${r.id}"><div class="list-main"><div class="list-title">${esc(r.content)}</div><div class="list-meta">${badge(r.kind)} ${badge(r.temporal_status,r.temporal_status==='current'?'good':'warn')} ${r.subjects?.map(s=>badge(s.name)).join('')||''} · ${when(r.updated_at)}</div></div>${r.open_review_count?badge(`${r.open_review_count} 审核`,'danger'):''}</div>`;
    if(libraryTab==='recent') return `<div class="list-row" data-open="recent" data-id="${r.id}"><div class="list-main"><div class="list-title">${esc(r.content)}</div><div class="list-meta">${badge(r.kind)} 强化 ${r.strength||0} 次 · 到期 ${when(r.expires_at)} · ${when(r.last_seen_at)}</div></div>${r.subject_id?badge('项目'):''}</div>`;
    if(libraryTab==='daily') return `<div class="list-row" data-open="daily" data-id="${r.id}"><div class="list-main"><div class="list-title">${esc(r.memory_date)} · ${esc(r.scope_key==='global'?'全局':r.scope_key)}</div><div class="list-meta">${esc(String(r.content||'').replace(/\n/g,' · ').slice(0,160))} · 到期 ${when(r.expires_at)}</div></div></div>`;
    return `<div class="list-row" data-open="history" data-id="${r.id}"><div class="list-main"><div class="list-title">${esc(r.content)}</div><div class="list-meta">${badge('v0.5 历史')} ${badge(r.status)} ${badge(r.kind)} · ${when(r.last_seen_at)}</div></div></div>`;
  }).join('')||empty('当前筛选没有条目');
}
...
async function renderProject(id) {
  currentProject=id; $$('[data-project]').forEach(b=>b.classList.toggle('active',b.dataset.project===id)); const [p,signals,daily,reflections]=await Promise.all([api(`/projects/${id}`),api(`/recent-signals?project_id=${encodeURIComponent(id)}`),api(`/daily-memories?project_id=${encodeURIComponent(id)}`),api('/rem-reflections?status=active')]); const summary=p.summary; const signalIds=new Set(signals.map(s=>s.id)); const projectReflections=reflections.filter(r=>(r.signal_ids||[]).some(x=>signalIds.has(x)));
  ...
  $("#project-workbench").innerHTML=`<div class="project-hero"><div><span class="eyebrow">${esc(p.status.toUpperCase())}</span><h2>${esc(p.name)}</h2><p class="muted">${esc(p.description||'未填写项目说明')}</p></div><div class="row"><button class="quiet small" data-context="${p.id}">上下文预览</button><button class="quiet small" data-summary="${p.id}">重建摘要</button><a class="button quiet small" href="${base}/projects/${p.id}/export" target="_blank">导出</a><button class="quiet small" data-edit-project="${p.id}">设置</button></div></div><article class="panel"><div class="panel-head"><div><span>SUMMARY</span><h2>项目摘要</h2></div>${summary?badge(summary.mode==='manual_override'?'手工锁定':'自动','good'):badge('未生成','warn')}</div><p class="muted">${summary?esc(summary.content):'尚无可追溯摘要；召回会回退到原子记忆和工作项。'}</p></article><div class="work-columns"><div class="work-column"><h3>当前状态</h3>${chips(grouped.current_state)}</div><div class="work-column"><h3>已确认决定</h3>${chips(grouped.decision)}</div><div class="work-column"><h3>开放问题</h3>${chips(grouped.open_question)}</div></div><details class="panel details"><summary>项目近期信号、Daily Memory 与 REM 主题</summary><h3>活跃近期信号</h3>${signals.map(s=>`<div class="work-chip" data-open="recent" data-id="${s.id}">${esc(s.content)} · 强化 ${s.strength||0}</div>`).join('')||empty('暂无')}<h3>项目 Daily Memory</h3>${daily.map(d=>`<div class="work-chip" data-open="daily" data-id="${d.id}">${esc(d.memory_date)} · ${esc(String(d.content).replace(/\n/g,' · '))}</div>`).join('')||empty('暂无')}<h3>REM 主题</h3>${projectReflections.map(r=>`<div class="work-chip">${badge(r.reflection_type)} ${esc(r.content)}</div>`).join('')||empty('暂无')}</details><details class="panel details"><summary>提案、里程碑与历史工作项</summary>${[...grouped.proposal,...grouped.milestone,...p.work_items.filter(w=>!['active'].includes(w.status))].map(w=>`<div class="work-chip" data-open="work" data-id="${w.id}">${badge(w.item_type)} ${badge(w.status)} ${esc(w.content)}</div>`).join('')||empty('暂无')}</details>`;
}

async function loadReviews() {
  const allowed=new Set(['conflict','sensitive','low_confidence','evidence_affected','deep_review']);
  const [decisionRows,suggestions]=await Promise.all([api('/reviews?status=open&queue=decision'),api('/reviews?status=open&queue=suggestion')]); const decisions=decisionRows.filter(r=>allowed.has(r.issue_type)); $("#decision-count").textContent=decisions.length; $("#suggestion-count").textContent=suggestions.length;
  const rows=reviewTab==='resolved'?(await api('/reviews?status=resolved')).filter(r=>allowed.has(r.issue_type)):reviewTab==='suggestion'?suggestions:decisions;
...
async function loadDreams(){const rows=await api('/dream-runs?limit=80');$("#dream-list").innerHTML=rows.map(r=>`<div class="list-row"><div class="list-main"><div class="list-title">${when(r.started_at)} · ${esc(r.status)}</div><div class="list-meta">输入 ${r.input_count||0} · 信号 ${r.recent_count||0} · Daily ${r.daily_count||0} · REM ${r.reflection_count||0} · 创建 ${r.promoted_count||0} · 更新 ${r.updated_memory_count||0} · 审核 ${r.review_count||0}</div>${r.error?`<div class="list-meta">${esc(r.error)}</div>`:''}</div>${badge(r.blocked_count?`${r.blocked_count} 阻塞`:'完成',r.blocked_count?'danger':'good')}</div>`).join('')||empty('暂无 Dream 日志');}
...
let hermesNative = {};
function renderHermesCounter(target) { const file=hermesNative[target]; const area=$(`#hermes-${target}-content`); const counter=$(`#hermes-${target}-counter`); if(!file||!area||!counter)return; const used=area.value.length; counter.textContent=`${used} / ${file.limit} 字符${used>file.limit?' · 超出上限':''}`; counter.className=used>file.limit?'error':'muted'; }
async function loadHermesNativeMemory(){
  const statusNode=$("#hermes-memory-status");
  try {
    const [user,memory]=await Promise.all([api('/hermes-native-memory/user'),api('/hermes-native-memory/memory')]); hermesNative={user,memory};
    for(const target of ['user','memory']){const area=$(`#hermes-${target}-content`);area.value=hermesNative[target].content;area.disabled=false;$(`#save-hermes-${target}`).disabled=false;renderHermesCounter(target);}
    statusNode.innerHTML=[['USER.md',user.exists?`存在 · ${when(user.modified_at)}`:'尚不存在，首次保存时创建'],['MEMORY.md',memory.exists?`存在 · ${when(memory.modified_at)}`:'尚不存在，首次保存时创建'],['隔离性','不会写入 B1ack 数据库、Dream 或检索']].map(([a,b])=>`<div class="health-row"><span>${esc(a)}</span><b>${esc(b)}</b></div>`).join('');
  } catch(error) {
    hermesNative={}; for(const target of ['user','memory']){const area=$(`#hermes-${target}-content`);area.value='';area.disabled=true;$(`#save-hermes-${target}`).disabled=true;$(`#hermes-${target}-counter`).textContent='不可用';}
    statusNode.innerHTML=`<div class="health-row"><span>当前 Hermes profile</span><b>${esc(error.message||'HERMES_HOME 未配置')}</b></div>`;
  }
}
async function saveHermesNativeMemory(target){const file=hermesNative[target];const area=$(`#hermes-${target}-content`);if(!file||!area)return; if(area.value.length>file.limit)throw new Error(`超过 ${file.limit} 字符上限`); const saved=await mutate(`/hermes-native-memory/${target}`,{content:area.value,expected_hash:file.content_hash},'PUT',false);hermesNative[target]=saved;renderHermesCounter(target);notice(`${target==='user'?'USER.md':'MEMORY.md'} 已保存；Hermes 会在下一会话读取。`);}
...
async function detail(type,id){
  ...
  if(type==='recent'){const r=(await api('/recent-signals?status=')).find(x=>x.id===id);if(!r)return;openDrawer(`<span class="eyebrow">RECENT SIGNAL</span><h2>${esc(r.content)}</h2><div class="list-meta">${badge(r.kind)} ${badge(r.status)} · 强化 ${r.strength||0} 次</div><div class="detail-section"><h3>来源</h3>${(r.evidence||[]).map(e=>`<div class="quote">${esc(e.excerpt||'无摘录')}<small>${when(e.observed_at)}</small></div>`).join('')||empty('暂无可见来源')}</div><p class="muted">到期：${when(r.expires_at)}。删除会立即停止其参与未来 Light、REM、Deep 与检索；不会静默删除长期记忆。</p><div class="detail-section row"><button class="danger" data-delete-recent="${r.id}">删除</button><button class="danger quiet" data-purge-recent="${r.id}">永久删除</button></div>`);}
  if(type==='daily'){const r=(await api('/daily-memories?status=')).find(x=>x.id===id);if(!r)return;openDrawer(`<span class="eyebrow">DAILY MEMORY</span><h2>${esc(r.memory_date)} · ${esc(r.scope_key==='global'?'全局':r.scope_key)}</h2><div class="detail-section"><pre>${esc(r.content)}</pre></div><p class="muted">到期：${when(r.expires_at)}。Daily Memory 是近期池的一部分，不是候选或长期记忆。</p><div class="detail-section row"><button class="danger" data-delete-daily="${r.id}">删除</button><button class="danger quiet" data-purge-daily="${r.id}">永久删除</button></div>`);}
  if(type==='history'){const rows=(await Promise.all(['pending','expired','rejected','promoted'].map(s=>api(`/candidates?status=${s}&admission_state=legacy_history`)))).flat();const r=rows.find(x=>x.id===id);if(!r)return;openDrawer(`<span class="eyebrow">FROZEN v0.5 HISTORY</span><h2>${esc(r.content)}</h2><p class="muted">此条目只保留浏览、血缘、导出与隐私删除兼容性，不参与 v0.6 Dream、默认召回或审核。</p><div class="detail-section"><h3>历史证据</h3>${(r.evidence||[]).map(e=>`<div class="quote">${esc(e.excerpt)}</div>`).join('')||empty('无')}</div>${r.status!=='promoted'?`<button class="danger" data-purge-history="${r.id}">永久删除历史</button>`:''}`);}
...
function confirmDialog(title,message){return new Promise(resolve=>{const d=$("#confirm-dialog");$("#confirm-title").textContent=title;$("#confirm-message").textContent=message;d.showModal();d.onclose=()=>resolve(d.returnValue==='confirm');});}
...
document.addEventListener('click',async event=>{const b=event.target.closest('button,[data-open]');if(!b)return;try{
  ...
  if(b.dataset.trashMemory){if(await confirmDialog('移入回收站','该操作可恢复，项目上下文将立即停止使用这条记忆。')){await mutate(`/memories/${b.dataset.trashMemory}/trash`);closeDrawer();}return;}
  ...
  if(b.dataset.purgeMemory){if(await confirmDialog('永久删除','将先创建隐私清理后的整包备份，并清理证据、召回、索引、实体关联及派生摘要。此操作不可撤销。')){await mutate(`/memories/${b.dataset.purgeMemory}`,{},'DELETE');closeDrawer();}return;}
  if(b.dataset.deleteRecent){if(await confirmDialog('删除近期信号','它将停止参与未来 Dream 与检索；相关长期记忆会保留。')){await mutate(`/recent-signals/${b.dataset.deleteRecent}`,{},'DELETE');closeDrawer();}return;}
  if(b.dataset.purgeRecent){if(await confirmDialog('永久删除近期信号','将清理关联证据、索引与适用隐私残留；若长期记忆证据受影响，会创建审核项。')){await mutate(`/recent-signals/${b.dataset.purgeRecent}?permanent=true`,{},'DELETE');closeDrawer();}return;}
  if(b.dataset.deleteDaily){if(await confirmDialog('删除 Daily Memory','它将停止参与未来 Dream 与检索；相关长期记忆会保留。')){await mutate(`/daily-memories/${b.dataset.deleteDaily}`,{},'DELETE');closeDrawer();}return;}
  if(b.dataset.purgeDaily){if(await confirmDialog('永久删除 Daily Memory','将清理关联证据、索引与适用隐私残留；若长期记忆证据受影响，会创建审核项。')){await mutate(`/daily-memories/${b.dataset.purgeDaily}?permanent=true`,{},'DELETE');closeDrawer();}return;}
  if(b.dataset.purgeHistory){if(await confirmDialog('永久删除历史候选','将执行历史隐私删除并更新干净备份。')){await mutate(`/candidates/${b.dataset.purgeHistory}`,{},'DELETE');closeDrawer();}return;}
  ...
  if(b.id==='save-hermes-user'){await saveHermesNativeMemory('user');return;}
  if(b.id==='save-hermes-memory'){await saveHermesNativeMemory('memory');return;}
