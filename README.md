...
- WebUI 查看、编辑、回收、永久删除记忆，管理待审核/已过期/已拒绝候选，追踪召回并完成设置、备份和维护。
...
从 v0.1.x 升级到 v0.2.0 时数据库会自动迁移。旧候选不会立即删除：超过无活动期限的记录会在下一次每日维护中先进入“已过期”，再从该时刻开始计算清理宽限期。升级前仍建议在 WebUI 创建一次手动备份。
...
REM 会在同一次已有模型调用中识别同义候选、已有长期记忆和临时噪声。同义项合并证据和召回记录；噪声进入已过期区。自动晋升要求模型置信度至少 0.80、最新 REM 已批准、无敏感或冲突，并满足以下任一通道：

- 重复证据通道：至少在 2 个不同日期获得证据。
- 实际作用通道：至少实际注入回答 2 次，且来自至少 2 种不同查询。

综合评分只负责给合格候选排序，不再是额外硬门槛。自动晋升每天最多 3 条，人工晋升不占用该额度。
